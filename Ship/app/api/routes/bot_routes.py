...
def call_bot_to_join_game(
    ...
    try:
        ...
        AuditLogger.log_game_action(
            user_id=current_user.user_id, game_id=game_id, action="call_bot", details={"bot_joined": True}
        )
        ...
        AuditLogger.log_authorization_failure(
            user_id=current_user.user_id, action="call_bot", resource=f"game:{game_id}", reason=str(e)
        )
...
def bot_attack_route(
    ...
    try:
        ...
        AuditLogger.log_game_action(
            user_id=current_user.user_id, game_id=game_id, action="bot_attack_triggered", details={}
        )
        ...
        AuditLogger.log_authorization_failure(
            user_id=current_user.user_id, action="bot_attack", resource=f"game:{game_id}", reason=str(e)
        )
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(e))
