...
from app.schemas.game import AttackData
...
def create_new_game(
    player1_id: int, current_user: User = Depends(get_current_user_from_token), db: Session = Depends(get_db)
):
    ...
    AuditLogger.log_game_access(user_id=current_user.user_id, game_id=new_game.game_id, action="create", success=True)
    return new_game
...
def join_existing_game(
    ...
    AuditLogger.log_game_access(user_id=current_user.user_id, game_id=game_id, action="join", success=True)
...
def start_game_route(
    game_id: int, current_user: User = Depends(get_current_user_from_token), db: Session = Depends(get_db)
):
    ...
    AuditLogger.log_game_action(
        user_id=current_user.user_id, game_id=game_id, action="start", details={"status": "started"}
    )
...
def get_all_games(
    ...
    AuditLogger.log_data_access(user_id=current_user.user_id, resource_type="games", resource_id="*", action="LIST")
    return games
...
def get_game_details(
    ...
    try:
        AuditLogger.log_data_access(
            user_id=current_user.user_id, resource_type="game", resource_id=game_id, action="READ"
        )
...
def surrender_game(
    ...
    try:
        ...
        AuditLogger.log_game_action(
            user_id=current_user.user_id, game_id=game_id, action="surrender", details={"surrendered_by": player_id}
        )
        return updated_game
    except NotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
