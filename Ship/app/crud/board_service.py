...
from app.schemas.board import BoardCreate, BoardFiltered
...
def create_board(db: Session, board_data: BoardCreate) -> BoardSchema:
    db_board = Board(game_id=board_data.game_id, player_id=board_data.player_id, board_state=board_data.board_state)
    db.add(db_board)
    db.commit()
    db.refresh(db_board)
...
def validate_board_lock(db: Session, board_id: int) -> bool:
    ...
    if not ships or len(ships) < 5:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="All 5 ships must be placed before locking the board"
        )

    if db_board.board_status == "locked":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Board is already locked")
...
def update_board_cell(db: Session, board_id: int, coordinates: List[int], value: str) -> Optional[BoardSchema]:
    ...
    AuditLogger.log_data_access(
        user_id=None, resource_type="board", resource_id=board_id, action="UPDATE_CELL"  # should be set by caller
    )
...
def filter_board_for_opponent(board: Board) -> BoardFiltered:
    ...
    filtered_board_state = [[cell if cell in ["H", "M"] else "O" for cell in row] for row in board.board_state]

    filtered_ships = []
    for ship in board.ships:
        is_sunk = len(ship.ship_hits) == len(ship.ship_coordinates)
        filtered_ships.append(ShipFiltered(ship_id=ship.ship_id, ship_type=ship.ship_type, is_sunk=is_sunk))
...
def get_board_for_player(db: Session, game_id: int, board_id: int, requesting_player_id: int) -> BoardSchema:
    from app.schemas.board import Board as BoardSchemaFull
