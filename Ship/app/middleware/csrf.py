    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if request.method in CSRF_PROTECTED_METHODS:
            ...
            if not csrf_cookie:
                ...
                return JSONResponse(status_code=403, content={"detail": "CSRF token missing in cookie"})

            if not csrf_header:
                logger.warning(f"CSRF: Missing header for {request.method} {request.url.path}")
                return JSONResponse(status_code=403, content={"detail": "CSRF token missing in header"})
