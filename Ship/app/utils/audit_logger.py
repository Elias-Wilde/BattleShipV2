...
class AuditLogger:
    ...
    def log_login_attempt(username: str, success: bool, ip_address: Optional[str] = None):
        ...
        audit_logger.warning(f"LOGIN_ATTEMPT | status={status} | username={username} | ip={ip_address}")
    ...
    def log_account_lockout(username: str, reason: str, ip_address: Optional[str] = None):
        ...
        audit_logger.critical(f"ACCOUNT_LOCKOUT | username={username} | reason={reason} | ip={ip_address}")

    @staticmethod
    def log_password_change(user_id: int, username: str, success: bool):
        """Log password changes"""
        status = "SUCCESS" if success else "FAILED"
        audit_logger.info(f"PASSWORD_CHANGE | status={status} | user_id={user_id} | username={username}")
    ...
    def log_game_access(user_id: int, game_id: int, action: str, success: bool):
        ...
        status = "SUCCESS" if success else "FAILED"
        audit_logger.info(f"GAME_ACCESS | status={status} | user_id={user_id} | game_id={game_id} | action={action}")
    ...
    def log_user_modification(user_id: int, modified_user_id: int, action: str, changes: Dict[str, Any]):
        ...
        audit_logger.info(
            f"USER_MODIFICATION | user_id={user_id} | modified_user_id={modified_user_id} "
            f"| action={action} | changes={changes_str}"
        )

    @staticmethod
    def log_security_event(
        event_type: str, details: Dict[str, Any], severity: str = "info", user_id: Optional[int] = None
    ):
        """Log general security events"""
        details_str = json.dumps(details)
        log_method = getattr(audit_logger, severity.lower(), audit_logger.info)
        user_part = f" | user_id={user_id}" if user_id else ""
        log_method(f"SECURITY_EVENT | type={event_type} | severity={severity} | details={details_str}{user_part}")
    ...
    def log_privilege_change(user_id: int, modified_user_id: int, old_role: str, new_role: str):
        ...
        audit_logger.critical(
            f"PRIVILEGE_CHANGE | user_id={user_id} | modified_user_id={modified_user_id} "
    ...
    def log_configuration_change(admin_id: int, config_key: str, old_value: str, new_value: str):
        ...
        audit_logger.critical(
            f"CONFIG_CHANGE | admin_id={admin_id} | config_key={config_key} "
    ...
    def log_authentication_attempt(
        ...
        audit_logger.warning(
            f"AUTH_ATTEMPT | status={status} | username={username} | user_id={user_id} "
            f"| reason={reason} | ip={ip_address} | timestamp={timestamp}"
        )
